# Scarecrovvs Realm Simulator v5
# - v4_fixed baseline + summaries + progress prints + VP piles + smarter bots
# - Workers/player = 2; Initiative capacity = 1; occupancy caps preserved.
# - Globals loaded from globals.csv. Only REAL moves are logged (no rollout logs).
...
class Card:
    ...
    effect: str = ""        # "draw:2", "hand_size_delta_next_round:+1", etc.
...
class Player:
    ...
    start_player_claims: int = 0
    field_visits: Dict[str,int] = field(default_factory=lambda:{f:0 for f in FIELDS})
    blocked_attempts: Dict[str,int] = field(default_factory=lambda:{f:0 for f in FIELDS})
...
class Config:
    ...
    # VP piles (buy costs); play cost is 0, VP card sits in deck like others
    vp_cost_1: int = 2
    vp_cost_2: int = 4
    vp_cost_3: int = 6
    # bot knobs
...
def act_buy_vp(g:Game,pid:int,vp:int)->bool:
    # buy a VP card into discard; play later from hand for points
    p=g.players[pid]
...
def place_worker(g:Game,pid:int,field:str)->bool:
    ...
    if field=="plasma": p.res["plasma"]+=1
        ...
        if p.hand:
            removed=p.hand.pop(0); g.emit({"t":g.turn,"a":"compost","p":pid,"card":removed})
    elif field=="initiative":
        g.round_mods.start_player=pid; p.start_player_claims+=1
        if g.pool:
...
def greedy_choose(g:Game,pid:int)->Tuple[str,Any]:
    ...
    for a,arg in acts:
        ...
        if a=="play":
            ...
            if tok.startswith("VP:"):
                vp=int(tok.split(":")[1])
                # prioritize VP more as urgency grows
            ...
            vp=arg
            # baseline value of buying VP scales with urgency and pile size
            v = (1.0+3.0*urgency)*vp
        elif a=="place_worker":
            f=arg
            base={"ash":2.0,"shards":2.0,"plasma":1.0,"forage":1.0,"rookery":1.2,"compost":0.6,"initiative":1.4}.get(f,0.5)
            # nudge to collect resources matching hand VP play-costs (VP play has no cost; nudge ash/shards anyway)
            if any(tok.startswith("VP:") for tok in p.hand):
                base += 0.2
            # deprioritize non-scoring workers late
            v = base - 0.6*urgency
        ...
        if v>bestv:
            best=(a,arg); bestv=v
    # Late-game safeguard: if very late, force VP-first choices when available
...
def rollout(g:Game,pid:int,horizon:int)->float:
    ...
    while steps<horizon and not is_terminal(g):
        ...
        steps+=1
    p=g.players[pid]
    # More VP weight in rollouts to finish games
...
def build_card_summary(outs:List[Dict[str,Any]], out_csv:str):
    ...
    all_ids = set(list(buys.keys())+list(plays.keys())+list(slots.keys()))
    ...
    with open(out_csv,"w",newline="",encoding="utf-8") as f:
        ...
        for cid in sorted(all_ids):
            ...
            ttf_med = (median(ttf.get(cid,[])) if ttf.get(cid,[]) else None)
            md = mat_dur.get(cid,[])
...
def build_field_summary(outs:List[Dict[str,Any]], out_csv:str):
    # Aggregate from per-game "players" state (field_visits/blocked) and logs (initiative)
    totals=[]
    for out in outs:
        # reconstruct some per-player counters
        # (We logged visits/blocked already on the Player object at runtime, but we don't serialize it,
        # so we approximate from logs.)
        visits=[{f:0 for f in FIELDS} for _ in range(6)]
        blocked=[{f:0 for f in FIELDS} for _ in range(6)]
        start_claims=[0]*6
        rounds_with_pf=0
        round_seen_pf=False
        current_turn=-1
        for e in out["log"]:
            if e["a"]=="place_worker":
                ...
                pass
            # detect new rounds by initiative_start_player
            if e["a"]=="initiative_start_player":
                if round_seen_pf:
                    rounds_with_pf+=1
                    round_seen_pf=False
...
def run_many(games:int=20, seed:int=42, **kwargs)->Dict[str,Any]:
    ...
    for i,out in enumerate(outs):
        path=f"logs/game_v5_{seed}_{i}.jsonl"
